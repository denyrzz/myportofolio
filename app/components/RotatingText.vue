...
const handleIndexChange = (newIndex: number): void => {
  ...
  const safeIndex = Math.max(0, Math.min(newIndex, props.texts.length - 1));
  currentTextIndex.value = safeIndex;
  props.onNext?.(safeIndex);
};
...
const startInterval = (): void => {
  if (typeof window !== 'undefined' && props.auto && props.texts.length > 1) {
    cleanupInterval();
...
watch(
  () => [props.auto, props.rotationInterval, props.texts.length] as const,
  () => {
    cleanupInterval();
    startInterval();
  },
