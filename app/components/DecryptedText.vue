  ...
  () => {
    ...
    const shuffleText = (originalText: string, currentRevealed: Set<number>): string => {
      if (props.useOriginalCharsOnly) {
        ...
        // FIX: Tambahkan type assertion untuk nonSpaceChars
        const nonSpaceChars = positions
          .filter(p => !p.isSpace && !p.isRevealed)
          .map(p => p.char) as string[];

        // FIX: Tambahkan check untuk array kosong
        ...
        return positions
          .map(p => {
            ...
            // FIX: Handle kasus nonSpaceChars kosong
            return nonSpaceChars[charIndex++] || availableChars[Math.floor(Math.random() * availableChars.length)];
          })
          .join('');
      } else {
        return originalText
